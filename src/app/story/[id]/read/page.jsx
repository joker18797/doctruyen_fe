...
export default function StoryReadPage() {
  ...
  const [unlockedChapters, setUnlockedChapters] = useState(() => {
    if (typeof window !== 'undefined') {
      const saved = localStorage.getItem('unlockedChapters_read')
  ...
  const showAdIfNeeded = (index, chapterId) => {
    ...
    const adKey = `hasShownAd_read_${id}_${chapterId}`
      ...
      try {
        ...
        if (res.status === 200) {
          ...
          if (!unlockedChapters.includes(chapterId)) {
            ...
            localStorage.setItem('unlockedChapters_read', JSON.stringify(updated))
          }
        }
      } catch (err) {
  ...
  const handleChangeChapter = (offset) => {
    ...
    router.push(`/story/${id}/read?chapter=${nextId}`)
  }
  ...
  return (
    <div>
      ...
      <div className="min-h-screen bg-gray-50 flex justify-center px-4 py-10">
        <div className="bg-white max-w-3xl w-full p-6 rounded-xl shadow-lg space-y-6">
          ...
          {chapterContent ? (
            ...
            <div className="text-center text-gray-500">Đang tải nội dung chương...</div>
          )}
          ...
          {hasLockedChapters && (
            <div className="mt-6 bg-[#FFEBCB] border border-yellow-300 rounded-xl p-6 shadow text-center">
              <p className="text-base text-gray-700">
                Mời đọc giả click vào nút <strong>"👉 Click để hiển thị"</strong> để mở khóa chương tiếp theo.
              </p>
              <p className="text-sm mt-2 text-gray-500 italic">
                (*) Bạn có thể được yêu cầu xem quảng cáo để mở khóa chương tiếp theo.
              </p>
            </div>
          )}
        </div>
